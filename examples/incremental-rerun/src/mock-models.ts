...
// demo is about the runtime's dedup, not a provider, so there is no API key and
// no provider SDK: each `call` sleeps for an artificial latency and returns an
...
function envelope(
  ...
  return {
    // Which generation of the `call` contract this adapter implements — the
    // dispatch path refuses an envelope it cannot execute rather than calling
...
export function createMockModels(options: MockModelsOptions = {}): MockModels {
  ...
  const textGeneration = envelope(
    ...
    async function mockTextGeneration<I, O>(input: I): Promise<DispatchResult<O>> {
      ...
      // Deterministic on purpose: the same prompt yields the same line, so a
      // reader can see step 2's input is step 1's output verbatim.
      const output: TextGenerationOutput = {
        modality: 'text',
        text: `Still of ${prompt}: centred in frame, soft morning light, shallow depth of field.`,
        cost: 0,
  ...
  const imageToVideo = envelope(
    ...
    async function mockImageToVideo<I, O>(
      ...
      // (threaded there by the meta through PatternRef.assets). Fail closed:
      // an image-to-video call with no start frame is a wiring bug, not a
      // text-to-video call.
      const startFrame = (ctx.assets ?? []).find((a) => a.slot === 'startFrame')
      if (!startFrame) {
        throw new Error(
          'image-to-video call: ctx.assets carries no startFrame — the meta must thread the still through PatternRef.assets',
        )
      }
      const startedAt = Date.now()
