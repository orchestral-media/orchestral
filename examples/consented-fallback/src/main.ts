...
let captureRoot: ((jobId: string) => void) | undefined
...
/**
 * Submit and return the terminal row either way. A terminal failure REJECTS
 * (`Runtime.submitJob`: the row is persisted with its `JobError`, `job:failed`
 * fires, then the error is rethrown) — so on rejection read the row back by
 * the id `onJobCreated` handed us. The first row created for a submit is the
 * root job; a meta's sub-steps create theirs afterwards.
 */
async function submit<TIn, TOut>(
  ...
  try {
    return await runtime.submitJob<TIn, TOut>(spec)
  } catch {
    if (!rootId) throw new Error('submitJob rejected before a job row existed')
    return runtime.pollJob<TIn, TOut>(rootId)
  }
}
...
// why it is unavailable, and every declared path that would have applied. The
// diagnostic names each path; the path's `losses` live on its declaration.
...
if (diagnostic) {
  ...
  for (const alt of diagnostic.alternatives) {
    const entry = declared.find((d) => d.id === alt.id)
    console.log(`    ${alt.id} -> ${alt.targetPatternId}`)
    console.log(`      keeps: ${spellOut(entry?.preserves)}. loses: ${spellOut(entry?.losses)}.`)
  }
...
interface RefusalDiagnostic {
  capability: string
  reason: string
  alternatives: readonly { id: string; description: string; targetPatternId: string }[]
  hint: string
}
