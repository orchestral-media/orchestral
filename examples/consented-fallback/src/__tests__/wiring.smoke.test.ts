...
describe('consented-fallback wiring', () => {
  ...
  it('default: refuses with ALTERNATIVES_NOT_ENABLED and names via-caption, calling no model', async () => {
    ...
    // A terminal failure rejects; the persisted row carries the JobError.
    await expect(
      runtime.submitJob({
    ...
    const job = await runtime.pollJob(jobIds[0]!)
    ...
    // The losses the demo prints come from the declaration the diagnostic names.
    expect(path.preserves).toEqual(['style'])
    expect(path.losses).toEqual(['subject-identity', 'composition', 'mask-guidance'])
