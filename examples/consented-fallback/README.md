...
Registration also prints four `[patterns] OUTPUTS_UNBOUNDED_FIELDS` advisories
to **stderr** — the registry's authoring lint on the shipped patterns'
`assets[].url` / `model` / `provider` fields. They are not part of the
narrative; the example's own meta declares a bounded output and adds none.
...
- **Step 2** reads the persisted job row. A terminal failure **rejects**
  `submitJob` (see `Runtime.submitJob` in `@orchestral/core`): the row is
  written with its `JobError`, `job:failed` fires, then the error is rethrown.
  `main.ts` catches, and reads the row back by the id `onJobCreated` captured.
  `error.details.diagnostic` names the capability, the reason, and every
  applicable path by `id` / `description` / `targetPatternId`; the `keeps` /
  `loses` line comes from the path's declaration, looked up on
  `registry.getEntry('image-to-image').alternatives` by that id.
